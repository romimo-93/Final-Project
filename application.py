...
def data():
    sql = sql_repo.MF_SQL_query("sql_data")
    if (sql != ""):
        results = sql_repo.MF_SQL(sql)
    return jsonify(results)


@application.route("/api/seasons")
def seasons():
    sql = sql_repo.MF_SQL_query("sql_seasons")
    if (sql != ""):
        results = sql_repo.MF_SQL_List(sql)
    return jsonify(results)
...
def daterequested(season, player_id, team_id):
    ...
    if ((player_id.isnumeric() == True) and (season.isnumeric() == True) & (team_id.isnumeric() == True)):        
        sql = sql_repo.MF_SQL_query("sql_daterequested",season, team_id, player_id)        
        if (sql != ""):
            results = sql_repo.MF_SQL(sql)    
...
def playerinfo(player_id):
    results = {}    
    sql = sql_repo.MF_SQL_query("sql_playerinfo", "", "", player_id)        
    if (sql != ""):
        results = sql_repo.MF_SQL(sql)    
    return jsonify(results)
...
def players():
    results = {}    
    sql = sql_repo.MF_SQL_query("sql_players") 
    if (sql != ""):
        results = sql_repo.MF_SQL(sql)    
    return jsonify(results)
...
def aggplayerstats(player_id):
    results = {}
    sql = sql_repo.MF_SQL_query("sql_aggplayerstats") 
...
def avgplayerstats(player_id):
    sql = sql_repo.MF_SQL_query("sql_avgplayerstats")     
    if (player_id.isnumeric()) == True & (sql != ""):  
        sql += " where player_id = " + player_id
    if (sql != ""):
        results = sql_repo.MF_SQL(sql)   
...
def teams():
    results = {}  
    sql = sql_repo.MF_SQL_query("sql_teams") 
    if (sql != ""):
        results = sql_repo.MF_SQL(sql)            
    return jsonify(results)
...
def seasonTeamPlayers(season, team_id):
    results = {}  
    sql = sql_repo.MF_SQL_query("seasonTeamPlayers", season, team_id)     
    if (sql != ""):
        results = sql_repo.MF_SQL(sql)   
    return jsonify(results)
...
def seasonTeams(season):
    results = {}
    sql = sql_repo.MF_SQL_query("seasonTeams", season)     
    print(sql)
    if (sql != ""):
        results = sql_repo.MF_SQL(sql)   
    return jsonify(results)    
